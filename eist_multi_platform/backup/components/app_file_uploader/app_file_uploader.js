...
export class AppFileUploader extends Component {
    ...
    async uploadAppFileInChunks(fileData) {
        ...
        try {
            ...
            // 检查版本是否存在
            const existingRecord = await this.checkVersionExists(file.name);
            if (existingRecord && !this.state.forceUpdate) {
                // 显示覆盖确认通知
                const message = sprintf(
                    _t('Version %s already exists for platform %s. Do you want to overwrite it?'),
                    existingRecord.version,
                    existingRecord.platform_type
                );

                await new Promise((resolve) => {
                    const notificationRemove = this.notification.add(message, {
                        title: _t('Version exists'),
                        type: 'warning',
                        sticky: true,
                        buttons: [
                            {
                            ...
                            },
                            {
                                ...
                                onClick: async () => {
                                    notificationRemove();
                                    this.notification.add(_t('Upload cancelled'), {
                                        type: 'info',
                                    });
                                    resolve(false);
                                },
            ...
            console.log("Odoo 默认最大文件上传大小", DEFAULT_MAX_FILE_SIZE / 1024 / 1024)
            
            // 根据文件大小决定是否需要分片
            if (file.size <= DEFAULT_MAX_FILE_SIZE) {
                console.log("直接上传",file.size)
                // 小文件直接上传
