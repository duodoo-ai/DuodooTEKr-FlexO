...
export class Overview extends Component {
    static template = "rtx_project.ProjectLargeScreen.Overview";
    static props = {
    ...
    setup() {
        ...
        onWillStart(async () => {
            try {
                ...
                if (this.state.shouldLoadJQ) {
                    await Promise.all([
                        loadJS('/rtx_project/static/libs/datatables/js/dataTables.min.js'),
                        loadCSS('/rtx_project/static/libs/datatables/css/dataTables.dataTables.min.css'),
                    ])
        ...
        onWillRender(() => {
            ...
            // if (this.state.shouldLoadJQ) {
            //     this.state.shouldLoadTable = Promise.all([loadJS('/rtx_project/static/libs/datatables/js/dataTables.min.js') && loadCSS('/rtx_project/static/libs/datatables/css/dataTables.dataTables.min.css')]).then(() => new Promise(resolve => window.grecaptcha.ready(() => resolve())));
            // }
        });
        ...
        onMounted(() => {
            ...
            if (this.state.shouldLoadTable) {
                this.table = new DataTable(this.tableRef.el, {
                    ...
                    language: {
                        url: sprintf(
                            "/rtx_project/static/libs/datatables/i18n/%s.json",
